...
export default function Home() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!count || count < 1) {
      ...
      // 驗證前 count 筆保險資訊必須全部填妥
      for (let i = 0; i < count; i++) {
        const p = participants[i];
        if (!p.name.trim()) {
          newErrors.push(`第 ${i + 1} 位參與者請填寫姓名`);
        }
        if (!p.idNumber.trim()) {
          newErrors.push(`第 ${i + 1} 位參與者請填寫身分證字號`);
        }
        if (!p.birthday.trim()) {
          newErrors.push(`第 ${i + 1} 位參與者請填寫生日`);
        }
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex justify-center items-center p-5">
      <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full overflow-hidden">
        ...
        {/* Form */}
        <form id="registrationForm" className="p-8" onSubmit={handleSubmit}>
          {/* Basic Info */}
                      ...
                      <div className="space-y-2">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-green-700"
                          />
                        </div>
                        <div>
                          <label className="text-xs text-gray-600 mb-1 block">
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-green-700 focus:ring-2 focus:ring-green-100 resize-none"
            />
          </div>
          {/* Contact Info */}
          ...
          <div className="flex gap-3 mt-8">
            <button
              type="submit"
              className="flex-1 bg-green-700 hover:bg-green-800 text-white font-semibold py-3 rounded-lg transition-colors duration-200"
            >
              ✓ 送出報名
            </button>
            <button
